...
- (void)viewDidLoad {
    ...
    [KSImagePickerController authorityCheckUpWithController:self type:KSImagePickerAuthorityTypePhotoLibrary completionHandler:^(KSImagePickerAuthorityType type) {
...
- (void)_done {
    if (_delegate != nil) {
        if ([_delegate respondsToSelector:@selector(imagePicker:didFinishSelectedAssetModelArray:mediaType:)]) {
            [_delegate imagePicker:self didFinishSelectedAssetModelArray:_selectedAsset mediaType:_mediaType];
        }
        if (_mediaType == KSImagePickerMediaTypePicture) {
            if ([_delegate respondsToSelector:@selector(imagePicker:didFinishSelectedImageArray:)]) {
                ...
                UIImage *nullImage = [UIImage imageNamed:@"KSUserInterface.bundle/icon_transparent"];
...
- (void)openSystemCamera {
    __weak typeof(self) weakSelf = self;
    if (_mediaType == KSImagePickerMediaTypePicture) {
        [KSImagePickerController authorityCheckUpWithController:self type:KSImagePickerAuthorityTypeCamera completionHandler:^(KSImagePickerAuthorityType type) {
        ...
        [KSImagePickerController authorityCheckUpWithController:self type:KSImagePickerAuthorityTypeCamera completionHandler:^(KSImagePickerAuthorityType type) {
            [KSImagePickerController authorityCheckUpWithController:self type:KSImagePickerAuthorityTypeMicrophone completionHandler:^(KSImagePickerAuthorityType type) {
                UIImagePickerController *cameraCtl = [[UIImagePickerController alloc] init];
                cameraCtl.sourceType = UIImagePickerControllerSourceTypeCamera;
...
- (void)imagePickerEditPicture:(KSImagePickerEditPictureController *)imagePickerEditPicture didFinishSelectedImage:(UIImage *)image assetModel:(KSImagePickerItemModel *)assetModel {
    if (_delegate != nil && image != nil && [_delegate respondsToSelector:@selector(imagePicker:didFinishEditImage:)]) {
        [_delegate imagePicker:self didFinishEditImage:image];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    [PHPhotoLibrary.sharedPhotoLibrary performChangesAndWait:^{
        ...
        if (@available(iOS 14.0, *)) {
            ...
            if ([type isEqualToString:UTTypeImage.identifier]) {
                UIImage *image = [info objectForKey:UIImagePickerControllerOriginalImage];
                request = [PHAssetChangeRequest creationRequestForAssetFromImage:image];
            } else if ([type isEqualToString:UTTypeMovie.identifier]) {
    ...
    if (itemModel != nil && !itemModel.isLoseFocus) {
        if (itemModel.isCameraCell) {
            ...
            [ctl setItemFrameAtIndex:^CGRect(NSUInteger index) {
                ...
                NSIndexPath *k_indexPath = [NSIndexPath indexPathForItem:index inSection:0];
                UICollectionView *k_collectionView = weakSelf.view.collectionView;
                KSImagePickerBaseCell *k_cell = (KSImagePickerBaseCell *)[k_collectionView cellForItemAtIndexPath:k_indexPath];
            ...
            [ctl setDidClickDoneButtonCallback:^(KSImagePickerViewerController *controller, NSUInteger index) {
                ...
                [controller dismissViewControllerAnimated:self->_isEditPictureStyle completion:^{
                    [weakSelf didClickDoneButton];
                }];
            }];
...
+ (void)authorityCheckUpWithController:(UIViewController *)controller type:(KSImagePickerAuthorityType)type completionHandler:(void(^)(KSImagePickerAuthorityType type))completionHandler cancelHandler:(void (^)(UIAlertAction *action))cancelHandler {
    if (controller != nil && completionHandler != nil) {
        switch (type) {
            case KSImagePickerAuthorityTypePhotoLibrary: {
                ...
                break;
            case KSImagePickerAuthorityTypeCamera: {
                ...
                break;
            case KSImagePickerAuthorityTypeMicrophone: {
                void(^authorization)(BOOL) = ^(BOOL granted) {
                    if (granted) completionHandler(type);
